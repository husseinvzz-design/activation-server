...
# ---------------- License check utilities (offline RSA-signed .lic) ----------------
import uuid, platform, hashlib, json, base64
from datetime import datetime
# cryptography is optional on client if you plan to use only public key verification.
# If cryptography is not installed on the client machine, verify_license will return (False, "missing_crypto").
try:
...
def get_hwid():
    """
    Compute a simple HWID using MAC and machine node name.
    Not bulletproof but reasonable for binding license to device.
    """
    try:
        mac = uuid.getnode()
        node = platform.node()
...
def verify_license(lic_path='license.lic', pubkey_path='public.pem'):
    """
    Verify a license file signed with the corresponding private key.
    Returns (True, data_dict) on success or (False, reason_str) on failure.
    """
    if not CRYPTO_AVAILABLE:
        return False, "missing_crypto"
    lp = Path(lic_path)
    if not lp.exists():
    ...
    try:
        ...
        return False, "invalid_license_payload"
    # expiry
    if 'expiry' in data:
        try:
    ...
    hwid = get_hwid()
    if data.get('hwid') and data.get('hwid') != hwid:
        return False, "hwid_mismatch"
    return True, data
# ---------------- End License utilities ----------------
...
def main():
    ...
    # --- License verification (offline RSA .lic) ---
    ok, info = verify_license('license.lic', 'public.pem')
    if not ok:
        if info == "missing_crypto":
            QMessageBox.critical(None, 'ترخيص مفقود/مكتبة مفقودة', 'يتطلب التحقق وجود مكتبة cryptography مثبتة.
لتثبيتها: pip install cryptography
أو ضع ملف الترخيص license.lic وملف public.pem في مجلد التطبيق.')
            return
        elif info == "no_license":
            QMessageBox.critical(None, 'ملف الترخيص مفقود', 'لم يتم العثور على ملف الترخيص (license.lic). يرجى وضع ملف الترخيص في مجلد التطبيق.')
            return
        elif info == "no_pubkey":
            QMessageBox.critical(None, 'مفتاح عام مفقود', 'لم يتم العثور على ملف public.pem. ضع ملف public.pem مع التطبيق.')
            return
        elif info == "expired":
            QMessageBox.critical(None, 'الترخيص منتهي', 'ترخيصك منتهي الصلاحية. اتصل بالمزوّد للحصول على ترخيص جديد.')
            return
        elif info == "hwid_mismatch":
            QMessageBox.critical(None, 'الترخيص ليس لهذا الجهاز', 'ملف الترخيص غير مرتبط بهذا الجهاز. يرجى الحصول على ترخيص مخصّص لهذا الجهاز.')
            return
        else:
            QMessageBox.critical(None, 'خطأ في الترخيص', f'تحقق من صلاحية ملف الترخيص: {info}')
            return
